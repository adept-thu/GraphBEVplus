...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def prepare_train_data(self, index):
        ...
        input_dict['pts_filename'] = './UniAD' + input_dict['pts_filename'].lstrip('.')
        input_dict['img_filename'] = ['./UniAD'+img_file.lstrip('.') for img_file in input_dict['img_filename']]
        for sweep in input_dict['sweeps']:
            if 'UniAD' in sweep['data_path']:
                continue
            sweep['data_path'] = './UniAD' + sweep['data_path'].lstrip('.')
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        logging.info('Results writes to', res_path)
    ...
    def evaluate(self,
        ...
        Args:
            ...
            logger (logging.Logger | str | None): Logger used for logging.infoing
        ...
        if isinstance(results, dict):
            if 'occ_results_computed' in results.keys():
                ...
                logging.info("Occ-flow Val Results:")
                for panoptic_key in out_metrics:
                    logging.info(panoptic_key)
                    # HERE!! connect
                    logging.info(' & '.join(
            ...
            planning_results_computed = None
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                logging.info('Evaluating bboxes of {}'.format(name))
        ...
        if isinstance(results_dict, dict):
            for key, value in results_dict.items():
                if key == "pts_bbox_NuScenes/mAP":
                    print(f"mAP\t{value:.3f}")
                elif key == "pts_bbox_NuScenes/amota":
                    print(f"amota\t{value:.3f}")
                elif key == "pts_bbox_NuScenes/amotp":
                    print(f"amotp\t{value:.3f}")
                elif key == "pts_bbox_NuScenes/NDS":
                    print(f"NDS\t{value:.3f}")
                elif key == "motion_metric":
                    print("Motion Metric:")
                    for k,v in value.items():
                        if k == "label_tp_errors":
                            print(f"min_ade_err\t{v['car']['min_ade_err']:.3f}")
                            print(f"min_fde_err\t{v['car']['min_fde_err']:.3f}")
                            print(f"miss_rate_err\t{v['car']['miss_rate_err']:.3f}")
                    # print_values(value)
        ...
        if 'map' in self.eval_mod:
            ...
            for key in key_list:
                ...
                map_result[key+'_mean'] = value / len(results)
                # print(''+key+'_mean:'+str( map_result[key+'_mean'] ))
            logging.info(results_dict)
        print("IoU Metrics:")
        for key, value in map_result.items():
            if key == "drivable_iou_mean":
                print(f"drivable_iou_mean\t{value:.3f}")
            elif key == "lanes_iou_mean":
                print(f"lanes_iou_mean\t{value:.3f}")
        
        if occ_results_computed is not None:
            print("Occupancy Metrics:")
            for key in occ_results_computed.keys():
                value = occ_results_computed[key]
                if key == "pq":
                    print(f"VPQ-n\t{value[0]:.3f}")
                    print(f"VPQ-f+\t{value[1]:.3f}")
                elif key == "iou":
                    print(f"IoU-n\t{value[0]:.3f}")
                    print(f"IoU-f+\t{value[1]:.3f}")

        # here we have planning
        if planning_results_computed is not None:
            print("Planning Metrics:")
            for key in planning_results_computed.keys():
                value = planning_results_computed[key]
                if key == "L2":
                    print(f"L2:\t{sum(value).item()/len(value):.3f}")
                elif key == "obj_box_col":
                    print(f"CR_avg\t{sum([value[0], value[2], value[4]]).item()*100/3:.3f}")
                elif key == "total_time_box_col":
                    print(f"CR_traj\t{value[-1].item()*100:.3f}")
    ...
    def _evaluate_single(self,
        ...
        if 'motion' in self.eval_mod:
            ...
            # here is motion metric
            motion_metric = self.nusc_eval_motion.main(
                                plot_examples=0,
                                render_curves=False,
                                eval_mode='standard')
            detail["motion_metric"] = motion_metric
